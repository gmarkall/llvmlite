...
struct RefPrunePass : public FunctionPass {
  ...
  bool findFanout(CallInst *incref, SmallBBSet &bad_blocks,
    ...
    if (findFanoutDecrefCandidates(incref, head_node, bad_blocks, decref_blocks,
                                   p_raising_blocks)) {
      if (DEBUG_PRINT) {
      ...
      if (prune_raise_exit) {
        ...
        // combine decref_blocks into raising blocks for checking the exit node
        // condition
  ...
  bool findFanoutDecrefCandidates(CallInst *incref, BasicBlock *cur_node,
    ...
    for (unsigned i = 0; i < term->getNumSuccessors(); ++i) {
      ...
      found = walkChildForDecref(incref, child, path_stack, subgraph_size,
